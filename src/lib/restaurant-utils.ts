...
export function buildInvoiceItems(items: DraftItem[], products: ProductLike[]) {
  ...
  return items.map((item) => {
    ...
    return {
      ...
      // Keep product master GST untouched, but restaurant bills remain tax-free.
      GSTPercentage: 0,
      GSTAmount: 0,
      total,
...
export function buildGstBreakup(
  _items: Array<{ total: number; GSTPercentage: number }>,
) {
  void _items;
...
export function buildCustomerSnapshot(
  input?: CustomerEntryInput,
  customerId?: string,
): InvoiceCustomerSnapshot | undefined {
  if (!input) {
    return undefined;
  }
